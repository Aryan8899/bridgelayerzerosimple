...
async function main() {
    ...
    if (!endpointAddress) {
        throw new Error("Endpoint address not found in deployment info. Check deployment structure.");
    }
    ...
    async function deployWithGasEstimation(contractName, args = [], description, maxRetries = 3) {
        for (let i = 0; i < maxRetries; i++) {
            try {
                ...
                console.log(`Estimated gas for ${description}: ${estimatedGas.toString()}`);
