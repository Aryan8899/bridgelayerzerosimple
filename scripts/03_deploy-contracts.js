...
async function main() {
    ...
    console.log("current recuever is",currentReceiver);
    if (currentReceiver === ethers.constants.AddressZero) {
        console.log("🔗 Setting WTAN receiver to:", receiver.address);
    
        try {
            const tx = await wtan.setReceiver(receiver.address, gasOptions);
            const receipt = await tx.wait();
            console.log(`✅ setReceiver() completed. Gas used: ${receipt.gasUsed.toString()}`);
        } catch (error) {
            console.error("❌ Failed to set receiver on WTAN:", error.message);
            throw error;
        }
    } else {
        console.log("⚠️ WTAN receiver already set to:", currentReceiver);
    }
